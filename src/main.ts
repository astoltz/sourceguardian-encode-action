...
export async function run(): Promise<void> {
  try {
    ...
    const filesToEncodeAsCustom = core.getMultilineInput(
    ...
    if (licenseRelease && licenseDynamic) {
      throw new Error(
        '`license-release` and `license-dynamic` cannot be used together.'
      )
    }
    const hasLockingOptions =
      domains.length > 0 ||
      domainEncrypt ||
      ips.length > 0 ||
      ipEncrypt ||
    ...
    if (external && hasLockingOptions) {
      throw new Error(
        '`external` cannot be used with other locking options (domain, ip, mac, machine-id).'
      )
    }
    if (external && (!projid || !projkey)) {
      throw new Error(
        '`projid` and `projkey` are required when using `external`.'
      )
    }
    if ((projid || projkey) && !external) {
      throw new Error(
        '`projid` and `projkey` can only be used with `external`.'
      )
    }
    ...
    if (machineIds.length > 0 && machineIdEncrypt) {
      throw new Error(
        '`machine-id` and `machine-id-encrypt` cannot be used together.'
      )
    }
